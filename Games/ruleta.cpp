...
void imprimirCanvas(int iluminado)
{
    ...
    for (int i = 0; i < NUMEROS; i++)
    {
        ...
        string color = (colores[i] == 'R') ? rojo : (colores[i] == 'N') ? negro
                                                                        : verde;

        // Si es el número iluminado se resalta con fondo blanco
        if (i == iluminado)
            color = fondo_blanco + color;
...
bool preguntarSeguir()
{
    ...
    while (true)
    {
        ...
        if (resp == "si" || resp == "sí")
            ...
            cout << "Respuesta inválida. Por favor, responde 'sí' o 'no'.\n";
    }
}
int main()
{
    ...
    while (saldo > 0)
    {
        ...
        for (int paso = 0; paso <= totalPasos; paso++)
        {
            ...
            if (paso > totalPasos - 10)
                esperarMs(200);
            else if (paso > totalPasos - 20)
                esperarMs(150);
            else
                esperarMs(60);
        }
        ...
        cout << (colorGanador == 'R' ? "Rojo" : colorGanador == 'N' ? "Negro"
                                                                    : "Verde")
             << ")\n";
