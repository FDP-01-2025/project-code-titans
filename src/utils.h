...
inline string trim(const string& str) {
    ...
    if (first == string::npos) return ""; // If it was all spaces, return an empty string
...
inline bool askToContinue() {
    ...
    while (true) {
        ...
        if (response == "si" || response == "yes") return true; // If they say yes, return true
        else if (response == "no") return false; // If they say no, return false
        else cout << "Invalid response. Please respond 'yes' or 'no'.\n"; // If they respond incorrectly, notify and repeat
    }
}
