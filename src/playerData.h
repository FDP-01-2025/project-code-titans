...
inline int loadBalance(const string& player) {
    ...
    if (!file.is_open()) return 0; // If it fails to open, return 0

    int savedMoney;
    file >> savedMoney; // Try to read the balance
    if (file.fail()) return 0; // If there was an error reading, return 0
...
inline bool loadStatistics(const string& name, int& played, int& won, int& lost, int& tied) {
    ...
    if (!file.is_open()) return false; // If I couldn't open it, return false
