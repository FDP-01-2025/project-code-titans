...
inline string adjustValue(int value) {
    ...
    if (value == 1) return "A";     // Ace is represented as "A"
    if (value == 11) return "J";    // Jack as "J"
    if (value == 12) return "Q";    // Queen as "Q"
    if (value == 13) return "K";    // King as "K"
...
inline void showCardsASCII(int values[], string suits[], int count, bool showAll = true) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (values[i] == 1) colorValue = "\033[34m";        // Blue for Ace
        else if (values[i] == 11) colorValue = "\033[36m";  // Cyan for Jack
        else if (values[i] == 12) colorValue = "\033[35m";  // Magenta for Queen
        else if (values[i] == 13) colorValue = "\033[33m";  // Yellow (gold) for King
        else colorValue = "\033[37m";                       // White for normal numbers
        ...
        if (showAll || i == 0) {  // Show all cards or only first card if hiding
...
inline void playBlackjack(Player& player) {
    ...
    do {
        ...
        while (true) {  // Loop to ask and validate difficulty
            ...
            if (!cin.fail() && difficultyLevel >= 1 && difficultyLevel <= 3) {  // If valid
                ...
                if (difficultyLevel == 1) printBlue("Normal level.\n");
                else if (difficultyLevel == 2) printYellow("Medium level.\n");
                else printRed("Hard level.\n");
                break;  // Exit the loop
            }
    ...
    cout << "\033[35m(Total: Hidden)\033[0m\n";  // I keep the dealer’s total hidden for now
        ...
        // Lambda function to play a single hand, to avoid code repetition for split hand
        auto playHand = [&](int values[], string suits[], int& numCards, int currentBet) -> bool {
            char option;  // I use this to store the player's choice (y/n)
            bool busted = false;  // I use this bool to check if I went over 21

            while (true) {
                int score = calculateScore(values, numCards);  // I calculate the current score

                if (score > 21) {  // If the player goes over 21, they automatically lose
                    cout << "\n\033[31mYou exceeded 21. You lost this hand.\033[0m\n";
                    player.money -= currentBet;  // Subtract the bet amount from the player's balance
                    player.gamesLost++;   // Increment lost games counter
                    registerGame("Blackjack", player.name, -currentBet, player.money);  // Register loss
                    saveBalance(player.name, player.money);  // Save new balance
                    player.updateStatistics();  // Update player statistics
                    busted = true;  // Mark that the player busted
                    break;  // Exit the loop to stop drawing cards
                }
                ...
                if (option != 'y') break;  // Exit loop if player says no
                ...
                values[numCards] = generateCard();  // Generate card's value
                suits[numCards++] = getRandomSuit();  // Generate suit of the card
        ...
        // Play the main hand
        bool playerBusted = playHand(playerValues, playerSuits, numPlayer, bet);
        ...
        if (hasSplit && !playerBusted) {
            ...
            splitBusted = playHand(splitValues, splitSuits, splitNum, bet);
        }
        ...
        if (!playerBusted) resolveHand(calculateScore(playerValues, numPlayer), bet);

        // Resolve split hand if any and not busted
        if (hasSplit && !splitBusted) resolveHand(calculateScore(splitValues, splitNum), bet, true);
